        ...
        <div class="bg-white rounded-2xl shadow-lg p-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                <div
                    ...
                    <div class="px-5 pb-5">
                        <button onclick="openPaymentModal('{{ $subpackage->title }}', {{ $subpackage->offer_price }}, {{ $subpackage->id }})"
...
<div id="paymentModal" class="fixed inset-0 bg-black/50 hidden flex items-center justify-center z-50 p-4">
    <div class="bg-white rounded-3xl shadow-2xl w-full max-w-md p-6 relative">
        ...
        <form id="paymentForm" method="POST" action="{{ route('payment.store') }}" class="space-y-4">
            ...
            <input type="tel" name="mobile" placeholder="Enter your mobile number"
                class="w-full border rounded-xl px-4 py-2 mb-4 focus:outline-none focus:ring-2 focus:ring-indigo-500">

            <!-- Amount -->
            <label class="block text-sm font-medium text-gray-600 mb-1">Amount</label>
            <input type="number" name="amount" id="modalAmount" placeholder="Enter amount"
                class="w-full border rounded-xl px-4 py-2 mb-4 focus:outline-none focus:ring-2 focus:ring-indigo-500" readonly>
    ...
    <script>
        function openPaymentModal(title, amount, packageId) {
            ...
            // Set dynamic package title
 
            document.getElementById('modalPackageSubtitle').textContent = title; // Shows the package title

            // Pre-fill package title
            document.getElementById('modalPackageTitle').value = title;
            // Pre-fill amount
            document.querySelector('#paymentForm input[name="amount"]').value = amount;
        ...
        document.getElementById('paymentForm').addEventListener('submit', function(e) {
            ...
            const packageId = this.package_id.value;
            const packageTitle = document.getElementById('modalPackageSubtitle').textContent;
            ...
            console.log('Mobile:', mobile, 'Amount:', amount);
