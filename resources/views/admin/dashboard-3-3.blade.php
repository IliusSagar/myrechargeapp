...
<div class="row mt-4">
  ...
  @php
  $dataPackage = DB::table('packages')->where('status', 'active')->get();
  @endphp

    @foreach($dataPackage as $package)
    @php
    $packageId = $package->id;
    $packageDetails = DB::table('package_details')->where('package_id', $packageId)->get();
    $packageDetailsId = $packageDetails->first()->id ?? 0;
    $packageAmount = DB::table('package_orderls')->where('package_id', $packageDetailsId)->where('status', 'approved')->sum('amount');
    @endphp
    <div class="col-lg-3 col-md-6 col-12">
        <div class="card shadow-sm border-0">
            <div class="card-body text-center">
                <img src="{{ asset('storage/' . $package->image_icon) }}" alt="{{ $package->name }}" class="img-fluid mb-2" style="max-height: 50px;">
