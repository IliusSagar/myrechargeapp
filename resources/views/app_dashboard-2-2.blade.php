...
<div class="relative">
    ...
    <div id="balanceDropdown" 
        class="hidden absolute right-0 mt-2 w-56 bg-white border border-blue-300 rounded-lg shadow-lg z-50 p-4">
        <!-- Current Balance -->
        <div class="flex justify-between items-center mb-2">
            <span class="text-gray-600 font-medium">Current Balance (MVR):</span>
            <span class="text-blue-600 font-bold text-lg">{{ Auth::user()->account->balance ?? 0 }}</span>
        </div>
...
@endphp
        ...
        <div class="flex justify-between items-center">
            <span class="text-gray-600 font-medium">Total Deposit (MVR):</span>
            <span class="text-green-600 font-bold text-lg">{{ $totalDeposit ?? 0 }}</span>
        </div>
    ...
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 relative">
        ...
        <form method="POST" action="{{ route('password.change') }}">
            ...
            <input type="password" name="password_confirmation" required
                class="w-full border rounded-lg px-3 py-2 mb-5 focus:outline-none focus:ring-2 focus:ring-indigo-500">

            <div class="flex justify-end gap-3">
                <button type="button" onclick="closePasswordModal()"
                    class="px-4 py-2 rounded-lg bg-gray-200 hover:bg-gray-300 font-semibold">Cancel</button>
                <button type="submit"
                    class="px-4 py-2 rounded-lg bg-indigo-600 text-white hover:bg-indigo-700 font-semibold">Update Password</button>
            </div>
        </form>
    </div>
</div>
